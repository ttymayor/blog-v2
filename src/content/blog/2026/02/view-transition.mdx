...
不過，儘管看 YouTube 教學或很多文檔，可能都告訴你在 astro:after-swap 事件 applyTheme 就好，但 after-swap 時新 DOM 已經換上去了，在 applyTheme() 執行前會有一瞬間是預設樣式。用 before-swap 是因為可以在 swap 之前就把 theme 套好。

同時也可以使用 Lifecycle events attributes `newDocument` 檢查並套用主題，確保主題已套用。

所以可以嘗試看看在 swap 前還是後，哪個比較不會有延遲問題。
